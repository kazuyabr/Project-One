		...
		if (id == 118)
		{
			for (IntIntHolder gold : Config.BANKING_SYSTEM_GOLDCOIN)
			{
				if (activeChar.getInventory().getInventoryItemCount(57, 0) >= Config.BANKING_SYSTEM_ADENA)
				{
					...
					activeChar.sendMessage("Você não tem Adena suficiente para converter para "+  ItemTable.getInstance().getTemplate(gold.getId()).getName() +"(s), você precisa de " + Config.BANKING_SYSTEM_ADENA + " Adena.");
			}
		}
		else if(id == 119)
		{
			...
			for (IntIntHolder gold : Config.BANKING_SYSTEM_GOLDCOIN)
			{
				if (activeChar.getInventory().getInventoryItemCount(gold.getId(), 0) >= gold.getValue())
				{
					...
					activeChar.sendMessage("Você não tem nenhum "+ ItemTable.getInstance().getTemplate(gold.getId()).getName() +" para trocar por " + Config.BANKING_SYSTEM_ADENA + " de Adena.");
			}
		}

...
abstract public class OlympiadGameNormal extends AbstractOlympiadGame
{
	...
	protected final boolean checkBattleStatus()
	{
		...
		if (!Config.ALT_OLY_ALLOW_DUALBOX_OLY)
		{
			if (_playerOne.getPlayer().getClient() != null && _playerTwo.getPlayer().getClient() != null)
			{
				...
				if (ip1.equals(ip2))
				{
					_playerOne.getPlayer().sendMessage("Your IP is same with opponent player. The Match aborted.");
					_playerTwo.getPlayer().sendMessage("Your IP is same with opponent player. The Match aborted.");
					LOGGER.warn("OLYMPIAD: " + _playerOne.getPlayer().getName() + " and " + _playerTwo.getPlayer().getName() + ". SAME IP.");
					return false;
				}
			}
		}
